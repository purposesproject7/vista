...
const CompactRubricTable = ({ 
  ...
  const [expandedStudent, setExpandedStudent] = useState(students[0]?.id || null);
  ...
  return (
    <div className="p-4 bg-gradient-to-br from-blue-50 to-gray-50 space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-100">{student.rollNumber}</p>
                  </div>
                  {allMarksEntered && (
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    ...
                    <div className="text-3xl font-bold">{studentTotal.toFixed(2)}</div>
                  </div>
            ...
            {isExpanded && (
              <div className="p-4 space-y-4 bg-gray-50">
                {rubrics.map((rubric, rubricIdx) => {
                  ...
                  const currentValue = parseFloat(marks[student.id]?.[rubric.id]);
                  const selectedLevel = rubric.levels?.find(l => l.score === currentValue);
                    ...
                    >
                      ...
                      <div className="bg-gradient-to-r from-gray-100 to-gray-50 px-4 py-3 border-b-2 border-gray-200 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <h4 className="font-bold text-gray-900 text-base">{rubric.component}</h4>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-sm text-gray-600 font-semibold">Max:</span>
                      ...
                      <div className="p-4">
                        <div className="grid grid-cols-1 gap-3">
                          {rubric.levels?.map((level) => {
                            ...
                            const actualMark = getActualMark(level.score, rubric.maxMarks);

                            return (
                              <button
                                key={level.score}
                                type="button"
                                onClick={() => onMarkChange(student.id, rubric.id, level.score.toString())}
                              ...
                              >
                                ...
                                <div className="flex flex-col items-center gap-1 flex-shrink-0">
                                  <div className={`w-20 h-20 rounded-xl flex flex-col items-center justify-center font-bold border-2 ${
                                    ...
                                    <div className="text-xs opacity-75 font-semibold">{level.label}</div>
                                  </div>
                                  {isSelected && (
      ...
      <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-4">
        <p className="text-sm text-blue-900">
          <strong className="text-blue-700">💡 Quick Tip:</strong> Click on a student card to expand and enter marks. Select the appropriate score level for each rubric component.
        </p>
      </div>
    </div>
  );
