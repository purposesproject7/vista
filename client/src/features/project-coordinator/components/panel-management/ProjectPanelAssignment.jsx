// src/features/project-coordinator/components/panel-management/ProjectPanelAssignment.jsx
...
const ProjectPanelAssignment = () => {
  ...
  const handleAutoAssignment = async () => {
    ...
    try {
      ...
      if (response.success) {
        showToast(
          `Successfully assigned ${response.assignedCount || 0} projects to panels`,
          'success'
        );
        loadData(filters);
      } else {
        showToast(response.message || 'Failed to auto-assign panels', 'error');
      }
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-blue-900">
              Selected: {filters.year}-{parseInt(filters.year) + 1}, Semester {filters.semester}
            </p>
