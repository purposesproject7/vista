...
import { XMarkIcon, CheckCircleIcon, TrashIcon, UserIcon, InformationCircleIcon } from "@heroicons/react/24/outline";
import Button from "../../../../shared/components/Button";
import Input from "../../../../shared/components/Input";
import { fetchFaculty, updatePanelMembers } from "../../services/coordinatorApi";
...
const EditPanelModal = ({ panel, filters, onClose, onSuccess }) => {
  ...
  const loadAvailableFaculties = async () => {
    try {
      ...
      const response = await fetchFaculty({
        ...
        program: filters.program || filters.programCode,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updatePanelMembers(panel.id, {
        memberEmployeeIds: selectedFaculties,
      });

      showToast("Panel members updated successfully", "success");
      onSuccess();
    } catch (error) {
      console.error("Error updating panel members:", error);
      showToast(error.response?.data?.message || "Failed to update panel members", "error");
    } finally {
      ...
      <div className="w-full max-w-4xl bg-white rounded-xl shadow-xl flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center shrink-0">
          <h2 className="text-xl font-semibold text-gray-900 flex items-center">
            ...
            Edit Panel Members ({panel.panelName})
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-1 space-y-4">
            <div className="bg-blue-50 border border-blue-200 p-3 rounded-lg flex gap-3 text-blue-800 text-sm">
              <InformationCircleIcon className="w-5 h-5 shrink-0" />
              <p>Project Coordinators can edit the faculty members in a panel, but cannot change the panel name.</p>
            </div>
