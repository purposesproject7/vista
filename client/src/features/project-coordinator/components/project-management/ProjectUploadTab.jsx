// src/features/project-coordinator/components/project-management/ProjectUploadTab.jsx
        ...
        try {
            ...
            const enrichedData = parsedData.map(project => {
                ...
                return {
                    ...
                    department: filters.department || filters.program, // PC context usually has program
                    program: filters.program, // Explicitly pass program if available
                    academicYear: filters.year || filters.academicYear // PC filter usually uses 'year'
                };
            });
            ...
            if (failed > 0) {
                // Build a detailed error summary for the user
                ...
                const successMsg = response.message || `Successfully uploaded ${created} project${created !== 1 ? 's' : ''}`;
            ...
            setUploadStatus({ success: false, message: error.response?.data?.message || error.message || 'Failed to upload projects' });
        } finally {
    ...
    const handleSubmitSingleProject = async (e) => {
        ...
        try {
            ...
            const projectData = {
                ...
                school: filters.school,
                department: filters.department || filters.program,
            ...
            showToast(error.response?.data?.message || error.message || 'Failed to add project', 'error');
        } finally {
                            ...
                            <div className="p-4 space-y-4">
                                ...
                                {uploadStatus && (
                                    <div className={`p-3 rounded-lg flex items-start gap-2 ${
                                        ...
                                        <pre className="text-sm whitespace-pre-wrap font-sans break-words flex-1">{
                                            uploadStatus.message?.startsWith('⚠️')
                                                ? uploadStatus.message.replace(/^⚠️\s*/, '')
