...
const verify = async () => {
    try {
        ...
        for (const f of facultyData) {
            ...
            console.log(`Created Faculty: ${fac.employeeId}, Dept: ${fac.department}`);
        }
        ...
        if (!createdFaculties[0].department) {
            ...
            console.error("❌ FAILURE: Faculty has department: " + createdFaculties[0].department);
        }
        ...
        const facultyList = [createdFaculties[0].employeeId, createdFaculties[1].employeeId]; // Only use F1 and F2, exclude F3
        ...
        if (panels.length === 1) {
            ...
            const memberIds = await Faculty.find({ _id: { $in: members.map(m => m.faculty) } }).distinct("employeeId");
            console.log("Panel Members:", memberIds);

            if (memberIds.length === 2 && memberIds.includes(facultyList[0]) && memberIds.includes(facultyList[1])) {
