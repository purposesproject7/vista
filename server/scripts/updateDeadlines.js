...
const updateDeadlines = async () => {
  try {
    ...
    console.log(`Updating deadlines for ${school} - ${department} (${academicYear}) to ${newDeadline.toISOString()}...`);

    // 1. Update DepartmentConfig
    const config = await DepartmentConfig.findOne({ academicYear, school, department });
    ...
    const coordinators = await ProjectCoordinator.find({ academicYear, school, department });
    console.log(`Found ${coordinators.length} coordinators.`);

    for (const coordinator of coordinators) {
      for (const key in coordinator.permissions) {
        if (coordinator.permissions[key] && coordinator.permissions[key].deadline) {
